...
export default function ContactButton({ status, serverError }: { status: string | null, serverError: string | undefined }) {
    ...
    return (
        <>
            <button type="submit" disabled={pending} className=" text-background text-xl flex justify-center items-center font-bold iconGlow bg-primary rounded-full my-2 h-8 w-24 px-3 py-5">
                <AnimatePresence>
                    {pending ?
                        <motion.div
